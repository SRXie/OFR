...
class SlotAttentionMethod(pl.LightningModule):
    ...
    def validation_epoch_end(self, outputs):
        ...
        with torch.no_grad():
            ...
            obj_greedy_loss_nodup_hn_A = torch.cat([x/(avg_slot_norm*self.model.num_slots) for x in obj_greedy_losses_nodup_hn_A], 0) # this should be changed to log sum exp if there are more than one hard negatives
            obj_greedy_loss_nodup_hn_D = torch.cat([x/(avg_slot_norm*self.model.num_slots) for x in obj_greedy_losses_nodup_hn_D], 0)
