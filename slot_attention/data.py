...
class CLEVRDataModule(pl.LightningDataModule):
    def __init__(
        ...
        self.test_dataset = CLEVRAlgebraTestset(
...
class SPRITEDataset(Dataset):
    def __init__(
        ...
        data_root: str,
        max_num_images: Optional[int],
        sprite_transforms: Callable,
        max_n_objects: int = 10,
        split: str = "train",
    ):
        super().__init__()
        self.data_root = data_root
        self.sprite_transforms = sprite_transforms
        self.max_num_images = max_num_images
    ...
    def __getitem__(self, index: int):
        ...
        img = Image.open(image_path)
        img = img.convert("RGB")
        return self.sprite_transforms(img)
    ...
    def get_files(self) -> List[str]:
        paths: List[Optional[str]] = []
        total_num_images = 20000
        i = 0
        while (self.max_num_images is None or len(paths) < self.max_num_images) and i < total_num_images:
            image_path = os.path.join(self.data_path, f'{i:08d}'+'.png')
            assert os.path.exists(image_path), f"{image_path} does not exist"
            paths.append(image_path)
            i += 1
...
class SPRITEDataModule(pl.LightningDataModule):
    def __init__(
        ...
        self.train_dataset = SPRITEDataset(
            data_root=self.data_root,
            max_num_images=self.num_train_images,
            sprite_transforms=self.sprite_transforms,
            split="train",
            max_n_objects=self.max_n_objects,
        )
