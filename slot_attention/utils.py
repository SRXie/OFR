...
def compute_partition_loss(cat_slots, cat_indices, A_losses, D_losses, cos_sim=False):
    ...
    if not cos_sim:
        ...
        A_loss = torch.exp(-(torch.norm(slots_A_delta, 2, -1).sum(2))).sum(1)#+torch.exp(-torch.norm(slots_A-slots_B, 2, -1).sum(1))
    else:
...
def bipartite_greedy_loss(cat_slots, cat_indices, slots_E, slots_F, losses_AE, losses_DF, cos_sim=False):
    ...
    losses_AE.append(torch.exp(-greedy_loss_AE))
    losses_DF.append(torch.exp(-greedy_loss_DF))
